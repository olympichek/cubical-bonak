...
nightly Agda 2.9.0 source 9c1bd18, imports-cached νSet
checks average 0.33 → 0.30 s (three runs per variant), comparing the
preceding explicit-square draft with the five-path form. Cold library
and both example builds take 75.49 → 61.55 s (single samples; the
...
`nuset-mechanical-simplification`.
